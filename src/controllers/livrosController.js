...
export const postLivros = (request, response) => {
    ...
    const insertSql = /*sql*/ `INSERT INTO livros (livro_id, titulo, autor, ano_publicacao, genero, preco, disponibilidade) VALUES
...
export const getLivrosUnico = (request, response) => {
...
export const putLivros = (request, response) => {
    ...
    conn.query(sql, (err, data) => {
        ...
        titulo = "${titulo}", autor = "${autor}", ano_publicacao = "${ano_publicacao}", genero = "${genero}", preco = "${preco}", disponibilidade = "${disponibilidade}" WHERE livro_id = "${id}"`;
